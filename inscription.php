<?php
   ...
   if(isset($valider)){
      ...
      elseif($pass!=$repass) $erreur="Mots de passe non identiques!";
         ...
         $sel=$pdo->prepare("select Id_modo from Modo where nom_modo=? limit 1");
         $sel->execute(array($login));
         $tab=$sel->fetchAll();
         if(count($tab)>0)
            $erreur="Login existe déjà!";
         else{
            $ins=$pdo->prepare("insert into Modo(Id_modo,mdp_modo,mon_modo) values(?,?,?)");
            if($ins->execute(array($id,$pass,$nom)))
